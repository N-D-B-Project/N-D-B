...
module.exports = {
      ...
      /******/ [
        ...
        /***/ function (module, exports, __webpack_require__) {
          ...
          class BumpCommand extends cli_1.BaseCommand {
            ...
            resolveFullPackageName({ scope, name }) {
  ...
  Please upgrade to Yarn 2.`);
              ...
              const { project } = await core_1.Project.find(
                configuration,
                this.context.cwd
              );
              ...
              for (const workspace of project.workspaces) {
                ...
                const descriptors = [...dependencies.values()]
                  .filter((descriptor) =>
                    this.resolveFullPackageName(descriptor).match(
                      this.packages.join("|") || ".*"
                    )
                  )
                  .filter(
                    (descriptor) =>
                      !this.resolveFullPackageName(descriptor).match(
                        this.exclude.join("|") || null
                      )
                  );
                const packageNames = descriptors.map(
                  this.resolveFullPackageName
                );
                const cli = clipanion_1.Cli.from(
                  plugin_essentials_1.default.commands
                );
          ...
          BumpCommand.usage = clipanion_1.Command.Usage({
            ...
            details:
              "A Yarn 2 plugin for upgrading PnP-mode dependencies easily" +
              " with a dead-simple command and no waste of interactions.",
            examples: [
              ["Upgrade all dependencies", "yarn bump"],
              ["Upgrade only the lodash package", "yarn bump ^lodash$"],
              [
          ...
          __decorate(
            [clipanion_1.Command.String("--kind")],
            BumpCommand.prototype,
            "dependencyKind",
            void 0
          );
