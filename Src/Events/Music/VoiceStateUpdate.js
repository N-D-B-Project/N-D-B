...
module.exports = class voiceStateUpdateEvent extends BaseEvent {
    ...
    async run(client, oldState, newState) {
      ...
      if (!player) return;
      if (MusicConfig.settings.leaveOnEmpty_Channel.enabled && oldState && oldState.channel) {
            ...
            try {
              ...
              if (player && oldState.guild.channels.cache.get(player.voiceChannel).members.size === 1) {
                var embed = new MessageEmbed()
                  ...
                  .setDescription(`Eu Sai do Canal: ${client.channels.cache.get(player.voiceChannel).name} pois ele ficou vazio por: ${ms(config.settings.leaveOnEmpty_Channel.time_delay, { long: true })}`)
                  .setColor("#00c26f")
                  .setFooter(client.user.tag, client.user.displayAvatarURL());
                if (player.get(`afk-${player.get("playerauthor")}`) || player.get(`afk-${player.guild}`))
                  return client.channels.cache.get(player.textChannel).send(embed.setDescription(`Não irei sair do Canal pois o Modo AFK está Ativo ✔`)).then(msg => {
                    try {
                      msg.delete({
                        timeout: 4000
