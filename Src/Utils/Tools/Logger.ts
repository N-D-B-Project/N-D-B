...
export default class Logger {
  ...
  private universalLog(type: string, department: string, ...content: any) {
    ...
    process.stdout.write(
      type + ' ' +
      Chalk.white(' | ') + ' ' +
      Chalk.italic.dim(Moment().format(Tools.getMomentFormat('calendar', 'pt-BR'))) + ' ' +
      Chalk.cyan.bold(' - ') + ' ' +
      this.prependDepartment(department) + ' ' +
      Chalk.white(' ') + ' ' + content.map((c: any) => typeof c !== 'string' ? inspect(c) : c).join(' ') 
    )
  }
  ...
  public warn(department = this.defaultDepartment, ...content: any) {
    this.universalLog(Chalk.black.bgYellow(' WARN     '), department, ...content)
  }
  ...
  public database(department = this.defaultDepartment, ...content: any) {
    this.universalLog(Chalk.rgb(77, 179, 61).bgBlack(' DATABASE '), department, ...content)
  }

  public command(department = this.defaultDepartment, ...content: any) {
    this.universalLog(Chalk.rgb(95, 72, 169).bgGreen(' COMMANDS '), department, ...content)
  }
  
  public event(department = this.defaultDepartment, ...content: any) {
    this.universalLog(Chalk.black.bgRgb(169, 147, 72)(' EVENTS   '), department, ...content)
  }

  public music(department = this.defaultDepartment, ...content: any) {
    this.universalLog(Chalk.rgb(101, 174, 246).bgGreen(' MUSIC    '), department, ...content)
  }

  public process(department = this.defaultDepartment, ...content: any) {
    this.universalLog(Chalk.rgb(211, 250, 103).bgGray(' PROCESS  '), department, ...content)
  }

  public ready(department = this.defaultDepartment, ...content: any) {
    this.universalLog(Chalk.rgb(40, 247, 80).bgGray(' READY    '), department, ...content)
  }
}