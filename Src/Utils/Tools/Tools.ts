...
export default class Tools {
  ...
  async Timer(type: string, number: number, translateInfo: Discord.Message | Discord.CommandInteraction | Discord.GuildChannel) {
    ...
    minutes = await this.client.translate("Tools/Tools:Timer:Minutes", translateInfo),
    seconds = await this.client.translate("Tools/Tools:Timer:Seconds", translateInfo);
    switch (type) {
      case "normal":
        return ` ${time.hours ? time.hours > 10 ? time.hours : `0${time.hours}` : ""}${time.hours ? ":" : ""}${time.minutes ? time.minutes >= 10 ? time.minutes : `0${time.minutes}` : "00"}:${time.seconds ? time.seconds > 10 ? time.seconds : `0${time.seconds}` : ""}`;
      case "details":
        return ` 
