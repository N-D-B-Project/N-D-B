    ...
    async loadEvents() {
        return glob(`${this.directory}events/**/*.js`).then(events => {
            for (const eventFile of events) {
                ...
                if (!this.isClass(File)) throw new TypeError(`Evento: \`${name}\` Não Exportou uma Class`);
                const event = new File(this.client, name);
                if (!(event instanceof Event)) throw new TypeError(`Evento: \`${name}\` Não está em Events`);
