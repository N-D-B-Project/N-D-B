...
const { Guild, Profile } = require("../database/schemas");
...
module.exports = (client) => {
  ...
  client.createGuild = async (settings) => {
    ...
    return newGuild
      ...
      .then(
        console.log(
          `Default settings saved for guild "${merged.guildName}" (${merged.guildID})`
        )
      );
  };
  ...
  client.createProfile = async (profile) => {
    ...
    return newProfile
      ...
      .then(
        console.log(
          `New profile saved for user "${merged.username}" (${merged.userID})`
        )
      );
  };
  ...
  client.updateProfile = async (user, data) => {
    ...
    console.log(
      `Profile "${profile.username}" (${profile.userID}) updated: ${Object.keys(
        data
      )}`
    );
