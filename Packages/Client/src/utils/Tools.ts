...
export class Tools {
	public static getMomentFormat(mode: "time" | "calendar" = "calendar", lang = "pt-BR"): string {
		switch (mode) {
			case "calendar":
				if (lang === "pt-BR") return "DD/MM/YYYY | LTS";
				return "DD-MM-YYYY LTS";
	...
	public static checkOwner(config: ConfigService<Config>, target: string) {
		return config.getOrThrow<Config["Discord"]>("Discord").Client.Owners.includes(target);
	}

	public static checkGuild(config: ConfigService<Config>, target: string) {
		return (
			config.getOrThrow<Config["Discord"]>("Discord").Servers.NDCommunity === target ||
			config.getOrThrow<Config["Discord"]>("Discord").Servers.TestGuild === target
		);
	}
