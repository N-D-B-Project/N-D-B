...
export class i18nService implements Ii18nService {
  ...
  public Logger(): void {
    ...
    if (
      this.database.ConfigRepo().getOrThrow<Config["Debug"]>("Debug")
        .Translations
    )
  ...
  public async Guild(
    ...
    const lang = (await this.database.GuildRepo().get(info.guild.id)).Settings
      .Language;
  ...
  public async DM(
    ...
    const lang = (await this.database.UserRepo().get(user.id)).Settings
      .Language;
    return this.i18n.translate(this.fixKey(key), { args, lang });
  }
