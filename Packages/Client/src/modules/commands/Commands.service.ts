...
export class CommandsService implements ICommandsService {
  ...
  public async load(command: CommandsDiscovery): Promise<void> {
    ...
    if (Slash) {
      if (
        Slash?.type === "Main" &&
        !als.getStore()["SlashCommands"].get(Slash?.data.name)
      ) {
        try {
          ...
          const guildCommands = this.client.guilds.cache.get(
            this.database.ConfigRepo().getOrThrow<Config["Discord"]>("Discord")
              .Servers.NDCommunity
          )?.commands;
          ...
          throw new CommandError(
            `Error when Registering Application Commands: ${error}`
          );
        }
      }
      ...
      if (Slash?.type === "Sub" && !als.getStore()["SubCommands"].get(Sub)) {
        if (!Slash.name)
          throw new CommandError(
            `SubCommand don't have a name: ${command.getClass()}`
          );
  ...
  private resolveCommand(nameOrAlias: string) {
    return (
      ...
      this.database
        ...
        .getStore()
        ["LegacyCommands"].get(
          this.database.AlsRepo().getStore()["Aliases"].get(nameOrAlias)!
        )
    );
  }
}
