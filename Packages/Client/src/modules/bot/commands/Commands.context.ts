...
export class Context {
	...
	public author: User;
	public channel: PartialDMChannel | GuildTextBasedChannel | TextChannel | DMChannel;
	public guild: Guild;
	...
	private async configureContext(Additional: IAdditional) {
		...
		this.interaction = this.isSlash ? (this.context as CommandInteraction) : null;
		this.author = this.context instanceof Message ? this.context.author : this.context.user;
	...
	public getArg(name: string, position: number) {
		return position === -1
			...
			: ((this.isSlash
					? this.isSub
						? (this.args as Array<CommandInteractionOption>).find((arg) => arg.name === name).value
	...
	public async send(content: Content) {
		if (this.isSlash) {
			this.msg = await InteractionTools.reply(this.interaction, content, false);
			return this.msg;
		}
		this.msg = await MessageTools.send(this.channel, content);
		return this.msg;
	}
	...
	public async reply(content: Content) {
		...
		this.msg = await MessageTools.reply(this.message, content);
		return this.msg;
	}

	public async sendToUserDM(content: Content) {
		this.msg = await MessageTools.send(this.author, content);
		return this.msg;
	}
	...
	public async getChannel(client: Client, name: string, position: number) {
		...
		return await (await this.getGuild(client)).channels.fetch(arg.replace("<#", "").replace(">", ""));
	}
	...
	public async getRole(client: Client, name: string, position: number) {
		...
		return (await this.getGuild(client)).roles.fetch(arg.replace("<@&", "").replace(">", ""));
	}
